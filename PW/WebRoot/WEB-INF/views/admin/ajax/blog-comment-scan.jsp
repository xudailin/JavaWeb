...
<div class="comment-content">
	<div class="blog-info">
		<input type="hidden" id="cId" value="${currentCommentId}" />
		<div class="user-info clearifx" id="blogInfo"></div>
		<div style="margin-bottom: 5px"><i class="fa fa-comment-o"></i><strong style="margin-left: 5px">评论</strong></div>
		<div>
		<ul class="comment-list" id="comment-list">
			<li class="comment-item clearfix" id="cinfo"></li>
		</ul>
		</div>
...
(function($){
		$(function(){
			var cId = $("#cId").val();
			var list ;
			$.post('${staticPath}/after/scanBlogComment.html',{id:cId},function(data){
			...
			function showDate(){
				...
				if (tag=='收起') {
					...
					$.each(list,function(i,comment){
						...
						if (comment.id==cId) { dComcontent="<strong>"+comment.comcontent+"</strong>"}else{dComcontent=comment.comcontent};
						if (comment.super_id=='0'){
							$("#cinfo").append("<div class='comment-pic left'><img src='${staticPath}/"+comment.commenter.headimgurl+"' class='comment-img'/></div><div class='left'><div class='comment-item-name'><font color='blue'>"+comment.commenter.nickname+"</font></div><div class='comment-item-time'>"+cpubtime.toLocaleString()+"</div><div class='comment-item-content'>"+dComcontent+"</div></div>");
						}else{
							$("#cinfo").append("<div style='padding-left: 50px;'><span class='comment-item-name'>"+comment.commenter.nickname+"</span><span>回复</span><span class='comment-item-name'>"+comment.superComment.commenter.nickname+"</span><span class='comment-item-name'>:</span><span class='comment-item-content'>"+dComcontent+"</span></div>");
						}
					 });
				}else{
					$("#cinfo").empty();
